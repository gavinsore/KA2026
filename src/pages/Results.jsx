                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="text-left border-b border-charcoal-200">
                                            <th className="pb-3 text-charcoal-600 font-medium">Date</th>
                                            <th className="pb-3 text-charcoal-600 font-medium">Round</th>
                                            <th className="pb-3 text-charcoal-600 font-medium">Archer</th>
                                            <th className="pb-3 text-charcoal-600 font-medium">Bow</th>
                                            <th className="pb-3 text-charcoal-600 font-medium text-right">Score</th>
                                            <th className="pb-3 text-charcoal-600 font-medium text-right">Hits</th>
                                            <th className="pb-3 text-charcoal-600 font-medium text-right">Golds</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {outdoorResults.map((result, index) => (
                                            <tr key={index} className="border-b border-charcoal-800 hover:bg-white/50 transition-colors">
                                                <td className="py-3 text-charcoal-600">{formatDate(result.date)}</td>
                                                <td className="py-3 text-forest-800">{result.event}</td>
                                                <td className="py-3 text-forest-900 font-medium">{result.archer}</td>
                                                <td className="py-3">
                                                    <span className={`px-2 py-1 rounded text-xs font-medium ${result.bowType === 'Recurve' ? 'bg-forest-100 text-forest-600' :
                                                            result.bowType === 'Compound' ? 'bg-gold-100 text-gold-600' :
                                                ...
                                                <td className="py-3 text-right text-gold-600 font-semibold">{result.score}</td>
                                                <td className="py-3 text-right text-charcoal-600">{result.hits}</td>
                                                <td className="py-3 text-right text-charcoal-600">{result.golds}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="text-left border-b border-charcoal-200">
                                            ...
                                            <th className="pb-3 text-charcoal-600 font-medium">Cat.</th>
                                            <th className="pb-3 text-charcoal-600 font-medium">Holder</th>
                                            <th className="pb-3 text-charcoal-600 font-medium text-right">Score</th>
                                            <th className="pb-3 text-charcoal-600 font-medium text-right">Year</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {clubRecords.map((record, index) => (
                                            <tr key={index} className="border-b border-charcoal-800 hover:bg-white/50 transition-colors">
                                                ...
                                                <td className="py-3">
                                                    <span className={`px-2 py-1 rounded text-xs font-medium ${record.bowType === 'Recurve' ? 'bg-forest-100 text-forest-600' :
                                                            record.bowType === 'Compound' ? 'bg-gold-100 text-gold-600' :
                                                ...
                                                <td className="py-3 text-forest-900 font-medium">{record.archer}</td>
                                                <td className="py-3 text-right text-gold-600 font-bold">{record.score}</td>
                                                <td className="py-3 text-right text-charcoal-600">{record.year}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div>
                            ...
                            <div className="grid gap-4">
                                {personalBests.map((pb, index) => (
                                    <div key={index} className="flex items-center justify-between p-4 bg-white/50 rounded-lg hover:bg-white transition-colors">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-forest-600 to-forest-700 flex items-center justify-center text-forest-900 font-bold">
                                                {pb.archer.split(' ').map(n => n[0]).join('')}
                                            </div>
                                            <div>
                                                <p className="text-forest-900 font-medium">{pb.archer}</p>
                                                <p className="text-charcoal-600 text-sm">{pb.round} • {formatDate(pb.date)}</p>
                                            </div>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-gold-600 text-xl font-bold">{pb.score}</p>
                                            <p className="text-forest-600 text-sm font-medium">{pb.improvement}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
