      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-charcoal-400 mt-0.5">
            Cookie-free · Last 30 days · Public pages only
          </p>
        </div>
      </div>
      ...
      {!loading && !error && (
        <>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {[
              ...
              <div key={label} className="glass-card p-5 flex flex-col gap-1">
                ...
                <span className="text-xs text-charcoal-400 font-medium uppercase tracking-wide">
                  {label}
                </span>
                <div className="mt-1 pt-1 border-t border-charcoal-100 flex items-center gap-1">
                  ...
                  <span className="text-xs text-charcoal-500">
                    <strong>{sessions.toLocaleString()}</strong> unique visitor
                    {sessions !== 1 ? "s" : ""}
                  </span>
                </div>
              </div>
          ...
          <div className="glass-card p-6 mb-6">
            ...
            {totalMonth === 0 ? (
              <p className="text-center text-charcoal-400 text-sm py-8">
              ...
              <div
                ...
                aria-label="Daily visits bar chart"
              >
                {dailyData.map(({ date, count }) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${fmtDate(date)}: ${count} visit${
                        count !== 1 ? "s" : ""
                      }`}
                    >
                      {/* Tooltip */}
                      <div
                        className="absolute bottom-full mb-1 hidden group-hover:flex flex-col items-center z-10 pointer-events-none"
            ...
            <div className="flex justify-between mt-2 text-xs text-charcoal-400">
              {[0, 7, 14, 21, 29].map((i) => (
                <span key={i}>
                  {dailyData[i] ? fmtDate(dailyData[i].date) : ""}
                </span>
              ))}
            </div>
            <div className="flex items-center gap-4 mt-3 text-xs text-charcoal-400">
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm bg-gradient-to-t from-forest-500 to-forest-400 inline-block" />{" "}
                Past days
              </span>
              <span className="flex items-center gap-1">
                <span className="w-3 h-3 rounded-sm bg-gradient-to-t from-gold-500 to-gold-400 inline-block" />{" "}
                Today
              </span>
            </div>
          </div>
                ...
                {topPages.map(({ page, count }, idx) => {
                  ...
                  return (
                    <div key={page} className="flex items-center gap-3">
                      <span className="text-xs text-charcoal-400 w-4 text-right shrink-0">
                        {idx + 1}
                      </span>
                      <span
                        className="text-sm font-medium text-charcoal-700 w-28 shrink-0 truncate"
                        title={page}
                      >
                        {pageName(page)}
                      </span>
...
const Dashboard = () => {
  ...
  const modules = [
    {
      ...
      description:
        "Manage upcoming club shoots, beginners courses, and social events.",
      icon: (
        <svg
    ...
    },
    {
      ...
      description:
        "Update the adult and junior fees shown on the beginners enrollment page.",
      icon: (
        <svg
    ...
    },
    {
      title: "Competitions",
      description:
        "Create and edit open competitions and download entry lists.",
      icon: (
        <svg
    ...
    },
    {
      ...
      description:
        "Upload PDF/CSV results files and manage the results archive.",
      icon: (
        <svg
    ...
    },
    {
      ...
      description:
        "Review and correct scraped NCAS county records. Fix round name mismatches and trigger manual data refreshes.",
      icon: (
        <svg
          className="w-8 h-8 text-purple-600"
          fill="none"
    ...
    },
    {
      ...
      icon: (
        <svg
          className="w-8 h-8 text-charcoal-600"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
  ...
  return (
    <div className="min-h-screen py-10 md:py-16">
      <SEO
        title="Admin Dashboard | Kettering Archers"
